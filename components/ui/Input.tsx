  ...
  ({ label, error, hint, className = '', id, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-2">
        {label && <label htmlFor={inputId} className="text-[12px] font-semibold text-white/50 uppercase tracking-[0.08em]">{label}</label>}
        <input ref={ref} id={inputId}
          className={[
            'w-full px-4 py-3 text-[14px] rounded-xl text-white placeholder-white/30',
